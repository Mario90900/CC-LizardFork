...
/obj/effect/weather/tornado	//standard tornado, throws people a distance
	...
	icon = 'modular_ratwood/icons/roguetown/effects/224x224.dmi'
	icon_state = "Tornado"
	pixel_x = -100
	pixel_y = -32
	plane = -2
	...
	var/drift_delay = 2 SECONDS
	var/next_drift_time
	var/current_drift_dir = SOUTH
	var/next_dir_change = 0
	var/sound/tornado_loop = 'modular_ratwood/sound/weather/tornado/tornado.ogg'
	var/sound_volume = 75
	var/sound_range = 20
...
/obj/effect/weather/tornado/proc/apply_tornado_force(atom/movable/A)
	...
	if(ishuman(A))
		...
		to_chat(H, span_big("You are thrown about by [src]!"))
...
/obj/effect/weather/tornado/abyssors_rage/process()
	..()
	//do_destruction() //Caustic - Lets... not have it destroy things.
